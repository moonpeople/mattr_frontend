...
const normalizeOptions = (raw: unknown): SwitchOption[] => {
  const parsed = normalizeArray<SwitchOption | string>(parseMaybeJson(raw), [])
  if (parsed.length === 0) {
    return []
  }
  ...
  return (parsed as SwitchOption[]).map((item) => ({
    label: String(item.label ?? item.value ?? ''),
    value: String(item.value ?? item.label ?? ''),
  }))
}

export const SwitchGroupWidget: WidgetDefinition<SwitchGroupProps> = {
  type: 'SwitchGroup',
  label: 'Switch Group',
  category: 'inputs',
  description: 'Multiple switches',
  defaultProps: {
    label: 'Label',
    value: '[]',
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {safeOptions.map((option) => {
            ...
            return (
              <label key={option.value} className="flex items-center justify-between gap-3 text-sm text-foreground">
