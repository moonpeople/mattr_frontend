...
const SwitchGroupRenderer = ({
  ...
  const options = useMemo<SwitchGroupOption[]>(
    () =>
      ...
      {options.map((option) => (
        ...
        >
          ...
          <Switch
            ...
            size="small"
            onCheckedChange={(nextChecked) => toggleValue(option.value, Boolean(nextChecked))}
          />
        </label>
      ))}
    </div>
...
export const SwitchGroupDefinition = createWidgetDefinition<SwitchGroupProps>({
  type: 'SwitchGroup',
  label: 'Switch Group',
  category: 'inputs',
  ...
  render: (props, context) => <SwitchGroupRenderer props={props} context={context} />,
})
