...
const formatTooltipValue = (
  value: number,
  format: RangeSliderProps['tooltipFormat'],
  decimals: number
) => {
...
export const RangeSliderDefinition = createWidgetDefinition<RangeSliderProps>({
  type: 'RangeSlider',
  label: 'Range Slider',
  category: 'inputs',
  ...
  render: (props, context) => {
    ...
    const stateStart = toNumber(context?.state?.start, props.start)
    const stateEnd = toNumber(context?.state?.end, props.end)
    const fallbackValues: [number, number] = [stateStart, stateEnd]
    const rawValues = Array.isArray(stateValues) && stateValues.length >= 2 ? stateValues : fallbackValues
    const min = props.min
    const max = props.max
    const step = Math.max(1, toNumber(props.step, 1))
