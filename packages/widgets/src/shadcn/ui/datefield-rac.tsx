...
function DateField<T extends DateValueRac>({
  ...
  return (
    <DateFieldRac
      className={composeRenderProps(className ?? '', (className: string) => cn(className))}
      {...props}
    >
      {children}
    </DateFieldRac>
...
function DateInput({
  ...
  return (
    <DateInputRac
      ...
      {...props}
    >
      {(segment: DateSegmentProps['segment']) => <DateSegment segment={segment} />}
    </DateInputRac>
  )
}
