...
import { ProjectTypeSelector } from 'components/interfaces/ProjectCreation/ProjectTypeSelector'
...
const Wizard: NextPageWithLayout = () => {
  ...
  const { data: platformRegionsData } = usePlatformRegionsQuery({
    enabled: projectType === 'iot',
    staleTime: 1000 * 60 * 5,
  })
  const iotRegions =
    platformRegionsData?.regions?.filter((region) => region.enabled !== false) ?? []
  ...
  const {
    ...
    onSuccess: (res) => {
      ...
      const projectHref = getProjectHref({ ref: res.ref, projectType })
      track(
  ...
  const onSubmit = async (values: z.infer<typeof FormSchema>) => {
    ...
    if (!isIotProject && useOrioleDb && !availableOrioleVersion) {
  ...
  useEffect(() => {
    ...
    const isValid = iotRegions.some((region) => region.slug === current)
    if (!isValid) {
              ...
              <div className="divide-y divide-border-muted">
                ...
                {canCreateProject && (
                  <>
                    ...
                    {!isIotProject && canChooseInstanceSize && <ComputeSizeSelector form={form} />}

                    {!isIotProject && <DatabasePasswordInput form={form} />}
                    ...
                    {!isIotProject && <SecurityOptions form={form} />}
                    {!isIotProject && showAdvancedConfig && !!availableOrioleVersion && (
