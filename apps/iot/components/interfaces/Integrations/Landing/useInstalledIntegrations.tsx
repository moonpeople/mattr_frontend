...
export const useInstalledIntegrations = () => {
  ...
  const schemasList = Array.isArray(schemas) ? schemas : EMPTY_ARR
  const extensionsList = Array.isArray(extensions) ? extensions : EMPTY_ARR
  const wrappersList = useMemo(() => (Array.isArray(data) ? data : EMPTY_ARR), [data])

  const isHooksEnabled = schemasList.some((schema) => schema.name === 'supabase_functions')
    ...
    return allIntegrations
      .filter((integration) => {
        ...
        if (integration.id === 'stripe_sync_engine') {
          const stripeSchema = schemasList.find(({ name }) => name === 'stripe')
          return (
        ...
        if (integration.type === 'wrapper') {
          return wrappersList.find((w) => wrapperMetaComparator(integration.meta, w))
        }
        if (integration.type === 'postgres_extension') {
          return integration.requiredExtensions.every((extName) => {
            const foundExtension = extensionsList.find((ext) => ext.name === extName)
  ...
  }, [allIntegrations, extensionsList, isHooksEnabled, schemasList, wrappersList])
