...
import { useIotClickhouseSqlRunMutation } from 'data/iot/clickhouse-sql'
...
import { BASE_PATH, IOT_DEFAULT_API_KEY } from 'lib/constants'
...
export const SQLEditor = () => {
  ...
  const autoLimitRef = useRef<number | undefined>(undefined)

  const { mutate: execute, isPending: isExecuting } = useIotClickhouseSqlRunMutation({
    onSuccess(data) {
      if (id) {
        snapV2.addResult(id, data, autoLimitRef.current)

        if (showPrettyExplain && isExplainQuery(data)) {
          snapV2.addExplainResult(id, data)
    ...
    onError(error: any) {
      if (id) {
        ...
        snapV2.addResultError(id, error, autoLimitRef.current)
      }
    },
  })

  const { mutate: executeExplain, isPending: isExplainExecuting } = useIotClickhouseSqlRunMutation({
    onSuccess(data) {
      if (id) {
        snapV2.addExplainResult(id, data)
        setActiveUtilityTab('explain')
  ...
  const executeQuery = useCallback(
    async (force: boolean = false) => {
      ...
      if (editorRef.current !== null && !isExecuting && project !== undefined) {
        ...
        autoLimitRef.current = appendAutoLimit ? limit : undefined
        execute({ sql: formattedSql, apiKey: IOT_DEFAULT_API_KEY, params: null })
  ...
  const executeExplainQuery = useCallback(async () => {
    ...
    if (editorRef.current !== null && !isExplainExecuting && project !== undefined) {
      ...
      const explainSql = isExplainSql(sql) ? sql : `EXPLAIN ${sql}`

      executeExplain({ sql: explainSql, apiKey: IOT_DEFAULT_API_KEY, params: null })
    }
  }, [
    isDiffOpen,
    id,
