...
const CodeEditor = ({
  ...
  useEffect(() => {
    if (!monaco || language !== 'jq') {
      return
    }
    let cancelled = false

    void (async () => {
      const { JQLanguageDefinition } = await import('monaco-languages-jq')
      if (cancelled) {
        return
      }
      const existing = monaco.languages
        .getLanguages()
        .some((entry) => entry.id === 'jq')
      if (!existing) {
      ...
      // monaco-languages-jq provides only a Monarch tokenizer; no language config.
    })()
