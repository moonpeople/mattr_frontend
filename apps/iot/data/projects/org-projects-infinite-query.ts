...
import { PLATFORM_PROJECT_TYPE } from 'lib/constants'
...
async function getOrganizationProjects(
  ...
  const project_type = PLATFORM_PROJECT_TYPE || undefined

  const { data, error } = await get('/platform/organizations/{slug}/projects', {
    params: { path: { slug }, query: { limit, offset, sort, search, statuses, ...(project_type ? { project_type } : {}) } as any },
    signal,
    headers,
  })
