...
export const fetchHandler: typeof fetch = async (input, init) => {
  ...
  // TODO: Remove temporary stubs for local development once platform endpoints are available.
  if (
    requestUrl.includes('/api/incident-status') ||
    requestUrl.includes('/api/platform/projects/') && requestUrl.includes('/run-lints')
  ) {
  ...
  if (requestUrl.includes('/platform/pg-meta/')) {
    // TODO: Remove pg-meta stubbing once platform pg-meta endpoints are reachable.
    const body = requestUrl.includes('/query') ? [] : { data: [], error: null }
