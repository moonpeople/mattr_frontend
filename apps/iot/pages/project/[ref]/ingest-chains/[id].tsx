...
import {
  useIotIngestChainDeleteMutation,
  useIotIngestChainMetadataMutation,
  useIotIngestChainMetadataQuery,
  useIotIngestChainQuery,
  useIotIngestChainsQuery,
  useIotIngestChainTestMutation,
...
const normalizeRuleChainType = (_value?: string | null) => 'ingest'
...
const DEFAULT_METADATA_TEMPLATE = {
  ...
  nodes: [
    {
      additionalInfo: { layoutX: 360, layoutY: 160 },
...
const NODE_TEMPLATES: NodeTemplate[] = [
  ...
  {
    label: 'Ingest Output',
    value: 'Ingest.IngestOutputNode',
    section: 'action',
    meta: {
      type: 'Ingest.IngestOutputNode',
      name: 'Ingest Output',
      configuration: {},
    },
  },
...
const NODE_TYPE_DESCRIPTIONS_RU: Record<string, string> = {
  ...
  'Ingest.IngestOutputNode': 'Публикует сообщение в ingest-очередь.',
...
const RuleChainDetailPage: NextPageWithLayout = () => {
  ...
  const ruleChainId = isRouterReady
    ? parseQueryId(
        (router.query?.ingestChainId as string | string[] | undefined) ??
          (router.query?.id as string | string[] | undefined)
      )
    : undefined
  const modelId = isRouterReady
    ? router.query?.ingestChainId
  ...
  } = useIotIngestChainQuery(ruleChainId)
  const { data: ruleChains = [], isPending: isRuleChainsLoading } = useIotIngestChainsQuery()
  const modelGate = useMemo(() => {
    ...
    if (ruleChain.type && ruleChain.type !== 'ingest') return { status: 'mismatch' as const }
  ...
  const [ruleChainType, setRuleChainType] = useState('ingest')
  ...
  const {
    ...
    error: metadataError,
  } = useIotIngestChainMetadataQuery(ruleChainId)
  ...
  const handleRunTest = useCallback(
    async (overrideMessages?: TestMessageEntry[]) => {
      ...
      try {
        const result = await testRuleChain({ ingestChainId: ruleChainId, payload: requestPayload })
        ...
        const kafkaOutbound = outputSummary.outboundRequests.filter(
          (entry) => entry?.type === 'kafka' || entry?.type === 'ingest_output'
        )
  ...
  const handleDeleteRuleChain = useCallback(async () => {
    if (!ruleChainId) return
    try {
      await deleteRuleChain({ ingestChainId: ruleChainId })
      setDeleteModalOpen(false)
      setInfoPanelOpen(false)
      if (ref) {
        await router.push(`/project/${ref}/ingest-chains`)
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const allowedRelations =
      getMessageTypeSwitchRelations(selectedNode) || getAllowedRelationsForNodeType(nodeType)
    if (!chainRelation || !allowedRelations.includes(chainRelation)) {
      setChainRelation(allowedRelations[0] ?? 'Forward')
    }
  }, [selectedNodeId, ruleChains, chainTargetId, chainRelation, nodeType, selectedNode])
  ...
  const metadataMissing =
    metadataError instanceof Error &&
    metadataError.message.includes('ingest_chain_metadata_not_found')
    ...
    const message =
      modelGate.status === 'loading'
        ...
        : modelGate.status === 'missing'
          ? 'Open this ingest rule from a device model to edit it.'
          : modelGate.status === 'invalid'
            ? 'Invalid ingest rule id.'
            : modelGate.status === 'model-not-found'
              ? 'Device model not found. Open this ingest rule from a valid device model.'
              : modelGate.status === 'missing-chain'
                ? 'Ingest rule not found for the selected device model.'
                : 'This ingest rule is not associated with the selected device model.'
