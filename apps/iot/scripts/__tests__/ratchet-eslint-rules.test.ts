...
describe('ratchet-eslint-rules integration', () => {
  it('captures per-file counts when initializing baselines', () => {
    ...
    const eslintResults = buildEslintResults([
      { filePath: repoPath('apps/iot/src/a.ts'), rules: { 'no-console': 1 } },
      { filePath: repoPath('apps/iot/src/b.ts'), rules: { 'no-console': 2 } },
    ])
    ...
    expect(metadata.ruleFiles['no-console']).toEqual({
      [relativeToCwd('apps/iot/src/a.ts')]: 1,
      [relativeToCwd('apps/iot/src/b.ts')]: 2,
    })
  })
  ...
  it('reports offending files when regressions occur and metadata has per-file data', () => {
      ...
      JSON.stringify(
        {
          ...
          ruleFiles: {
            'no-console': {
              [relativeToCwd('apps/iot/src/a.ts')]: 2,
            },
          },
        },
        null,
    ...
    const eslintResults = buildEslintResults([
      { filePath: repoPath('apps/iot/src/a.ts'), rules: { 'no-console': 3 } },
      { filePath: repoPath('apps/iot/src/b.ts'), rules: { 'no-console': 1 } },
    ])
    ...
    expect(combinedErrors).toContain(`${relativeToCwd('apps/iot/src/a.ts')} (+1)`)
    expect(combinedErrors).toContain(`${relativeToCwd('apps/iot/src/b.ts')} (+1)`)
  })
  ...
  it('falls back gracefully when baseline is missing per-file data', () => {
    ...
    const eslintResults = buildEslintResults([
      { filePath: repoPath('apps/iot/src/a.ts'), rules: { 'no-console': 2 } },
    ])
    ...
    expect(combinedErrors).toContain(`${relativeToCwd('apps/iot/src/a.ts')} (2 current)`)
  })
})
