/**
 * Контрол обработчиков событий builder: редактирование event-handlers в inspector.
 */
...
const resolveOptionValue = (
  value: unknown,
  options: Array<{ value: string; [key: string]: unknown }>,
  fallback: string
) => {
...
const cloneEvent = (event: Record<string, unknown>) => {
  try {
...
export const BuilderEventHandlers = ({
  ...
  const resolvedEventOptions = eventOptions ?? defaultEventOptions
  const resolvedActionOptions =
    actionOptions && actionOptions.length > 0
      ? actionOptions
      : resolvedEventOptions.length > 0
        ? defaultActionOptions
        : []
  const canCreateHandlers =
    resolvedEventOptions.length > 0 && resolvedActionOptions.length > 0
  ...
  const openEditor = (index: number | null) => {
    if (!canCreateHandlers && index === null) {
      return
    }
    ...
    const defaultActionType =
      resolvedActionOptions[0]?.value ?? normalizeActionType(existing?.type)
    const nextEvent =
      existing ??
      ({
  ...
  const handleDuplicate = (index: number) => {
    const existing = normalizedEvents[index]
    if (!existing) {
      return
    }
    const nextEvents = [...normalizedEvents]
    nextEvents.splice(index + 1, 0, cloneEvent(existing))
    onChange(nextEvents)
  }

  const handleDelete = (index: number) => {
    const nextEvents = normalizedEvents.filter((_, idx) => idx !== index)
    onChange(nextEvents)
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-[12px] font-medium text-foreground">
        ...
        <button
          type="button"
          className="rounded-md p-1 px-1 text-foreground-muted hover:bg-foreground/10 hover:text-foreground"
          disabled={!canCreateHandlers}
      ...
      {normalizedEvents.length === 0 ? (
        <div className="rounded-md border border-foreground-muted/20 bg-surface-100 px-2 py-0.5 text-[11px] leading-4 text-foreground-muted">
          {resolvedEventOptions.length === 0
            ? 'This widget does not expose configurable events.'
            : resolvedActionOptions.length === 0
              ? 'No available actions for this widget.'
            : 'Trigger queries, control components, or call other APIs in response to component events.'}
        </div>
      ) : (
        <div className="space-y-1 rounded-md border border-foreground-muted/20">
          {normalizedEvents.map((event, index) => (
            <div
              key={`${event.event ?? 'event'}-${index}`}
              className="flex w-full items-center justify-between gap-2 rounded-md bg-surface-100 px-2 py-1.5 text-[12px] leading-4 hover:bg-surface-200"
            >
              <button
                type="button"
              ...
              <Popover_Shadcn_>
                ...
                <PopoverContent_Shadcn_ align="end" className="w-32 p-1">
                  <button
                    type="button"
                    className="flex w-full items-center rounded-sm px-2 py-1 text-xs text-foreground hover:bg-surface-200"
                  ...
                  <button
                    type="button"
                    className="flex w-full items-center rounded-sm px-2 py-1 text-xs text-destructive hover:bg-surface-200"
            ...
            <DialogSection className="space-y-4 pb-0" padding="small">
                  ...
                  {resolvedActionOptions.length > 0 ? (
                    <Select_Shadcn_
                      value={resolveOptionValue(
                        ...
                        resolvedActionOptions[0]?.value ?? 'controlComponent'
                      )}
                    ...
                    <Input_Shadcn_ value="No available actions" className="h-7 text-xs" readOnly />
                  )}
                </div>
              </div>
              ...
              {activeActionType === 'goToApp' && (
                <div className="space-y-3">
                  ...
                  {(!eventApps.length || Boolean(draftEvent.appUrl)) && (
                    <Input_Shadcn_
