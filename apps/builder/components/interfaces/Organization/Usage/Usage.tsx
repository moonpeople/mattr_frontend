...
export const Usage = () => {
  ...
  const billingCycleStart = useMemo(() => {
    ...
    return dayjs.unix(subscription.current_period_start).utc()
  }, [subscription?.current_period_start])

  const billingCycleEnd = useMemo(() => {
    if (!subscription?.current_period_end) return null
    return dayjs.unix(subscription.current_period_end).utc()
  }, [subscription?.current_period_end])

  const currentBillingCycleSelected = useMemo(() => {
    // Selected by default
    if (!dateRange?.period_start || !dateRange?.period_end) return true
    if (!billingCycleStart || !billingCycleEnd) return true
          ...
          <Admonition
            ...
            description={
              <div>
                ...
                <Link
                  href="/docs/guides/platform/manage-your-usage"
                  target="_blank"
                >
                  organization-level usage
                </Link>{' '}
                and quotas. For usage calculations, we sum up usage from all your projects. To view
