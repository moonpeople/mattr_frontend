...
export const ReportChartUpsell = ({ report, orgSlug }: ReportsChartUpsellProps) => {
  ...
  const requestedPlan =
    Array.isArray(report.availableIn) && report.availableIn.length > 0 ? report.availableIn[0] : 'Pro'
  const subject = 'Enquiry to upgrade plan for organization'
  const message = `Organization Slug: ${orgSlug || '_'}\nRequested plan: ${requestedPlan}`
    ...
    <Card className={cn('h-[280px] relative')}>
      <div className="z-10 flex flex-col items-center justify-center space-y-2 h-full absolute top-0 left-0 w-full bg-surface-100/70 backdrop-blur-md">
        ...
        >
          ...
          >
            Upgrade to <span className="capitalize">{requestedPlan}</span>
          </SupportLink>
        </Button>
      </div>
