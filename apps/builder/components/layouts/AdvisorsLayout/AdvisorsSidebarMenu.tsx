  ...
  return (
    <div className="pb-12 relative">
      <FeaturePreviewSidebarPanel
        className="mx-4 mt-4"
        title={t('advisorsMenu.panelTitle', 'Moving to the toolbar')}
        description={t(
          'advisorsMenu.panelDescription',
          'Advisors are now available in the top toolbar for quicker access across the dashboard.'
        )}
        illustration={<Badge variant="success">{t('advisorsMenu.new', 'New')}</Badge>}
        actions={
          <Button size="tiny" type="default" onClick={handleOpenAdvisor}>
            {t('advisorsMenu.tryItNow', 'Try it now')}
          </Button>
        }
      />

      <ProductMenu page={page} menu={generateAdvisorsMenu(project, { advisorRules }, t)} />
    </div>
  )
}
