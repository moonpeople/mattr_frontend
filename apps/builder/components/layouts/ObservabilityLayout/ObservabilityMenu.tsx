...
const ObservabilityMenu = () => {
  ...
  const { mutate: deleteReport, isPending: isDeleting } = useContentDeleteMutation({
    onSuccess: () => {
      ...
      toast.success(t('observabilityMenu.deleteReportSuccess', 'Successfully deleted report'))
      router.push(`/project/${ref}/observability`)
    },
    onError: (error) => {
      toast.error(
        `${t('observabilityMenu.deleteReportError', 'Failed to delete report')}: ${error.message}`
      )
    },
  })
  ...
  const menuItems = [
    {
      title: t('observabilityMenu.performanceReports', 'Performance Reports'),
      key: 'performance-reports',
      items: [
        {
          name: t('observabilityMenu.apiGateway', 'API Gateway'),
          key: 'api-overview',
          url: `/project/${ref}/observability/api-overview${preservedQueryParams}`,
        },
        {
          name: t('observabilityMenu.queryPerformance', 'Query Performance'),
        ...
        ...(postgrestReportEnabled
          ? [
              {
                name: t('observabilityMenu.dataApi', 'Data API'),
                key: 'postgrest',
    ...
    {
      title: t('observabilityMenu.productReports', 'Product Reports'),
      key: 'product-reports',
      items: [
        ...(authEnabled
          ? [
              {
                name: t('observabilityMenu.auth', 'Auth'),
                key: 'auth',
          ...
          : []),
        {
          name: t('observabilityMenu.database', 'Database'),
          key: 'database',
        ...
        ...(edgeFnEnabled
          ? [
              {
                name: t('observabilityMenu.edgeFunctions', 'Edge Functions'),
                key: 'edge-functions',
        ...
        ...(realtimeEnabled
          ? [
              {
                name: t('observabilityMenu.realtime', 'Realtime'),
                key: 'realtime',
        ...
        ...(storageEnabled
          ? [
              {
                name: t('observabilityMenu.storage', 'Storage'),
                key: 'storage',
    ...
    <Menu type="pills" className="mt-6">
      {isLoading ? (
        ...
        <div className="flex flex-col gap-y-6">
              ...
              title={
                <span className="flex w-full items-center justify-between relative h-6">
                  <span className="uppercase font-mono">
                    {t('observabilityMenu.customReports', 'Custom Reports')}
                  </span>
                      ...
                      tooltip={{
                        content: {
                          ...
                          text: !canCreateCustomReport
                            ? t(
                                'observabilityMenu.customReportsPermission',
            ...
            {reportMenuItems.length === 0 ? (
              <div className="px-2">
                <InnerSideBarEmptyPanel
                  title={t('observabilityMenu.noCustomReports', 'No custom reports yet')}
                  description={t(
                    'observabilityMenu.noCustomReportsDescription',
                  ...
                  actions={
                    ...
                    >
                      {t('observabilityMenu.newCustomReport', 'New custom report')}
                    </ButtonTooltip>
                  }
                />
              </div>
          ...
          <ConfirmationModal
            title={t('observabilityMenu.deleteReportTitle', 'Delete custom report')}
            confirmLabel={t('observabilityMenu.deleteReportConfirm', 'Delete report')}
            confirmLabelLoading={t('observabilityMenu.deleteReportLoading', 'Deleting report')}
            size="medium"
            loading={isDeleting}
          ...
          >
            <div className="text-sm text-foreground-light grid gap-4">
              <div className="grid gap-1">
                <p>
                  {t(
                    'observabilityMenu.deleteReportPrompt',
                    "Are you sure you want to delete '{name}'?"
                  ).replace('{name}', selectedReportToDelete?.name ?? '')}
                </p>
              </div>
            </div>
          </ConfirmationModal>
