...
export function useSqlEditorGotoCommands(options?: CommandOptions) {
  ...
  useRegisterCommands(
    ...
    [
      {
        id: 'nav-sql-editor',
        name: t('sqlCommands.sqlEditor', 'SQL Editor'),
...
export function useSnippetCommands() {
  ...
  const { t } = useI18n()
  const snippetPageName = t('sqlCommands.snippets', 'Snippets')
  ...
  useRegisterCommands(
    ...
    [
      {
        id: 'run-snippet',
        name: t('sqlCommands.runSnippet', 'Run snippet'),
        icon: () => <Code />,
        action: () => setPage(snippetPageName),
      },
    ],
    {
      enabled: !!project,
  ...
  return (
    <div className="p-6">
      <p className="text-center">
        ...
        {t('sqlCommands.loadSnippetsError', "Couldn't load snippets")}
      </p>
    </div>
  )
}
...
function EmptyState({
  ...
  return (
    <div className="p-6">
      <p className="mb-2 text-center">{t('sqlCommands.noSnippets', 'No snippets found.')}</p>
      <CommandList_Shadcn_ className="py-2">
        <CommandGroup_Shadcn_>
          ...
          >
            {canCreateNew
              ? t('sqlCommands.createSnippet', 'Create new snippet')
              : t('sqlCommands.runNewSql', 'Run new SQL')}
          </CommandItem_Shadcn_>
        </CommandGroup_Shadcn_>
      </CommandList_Shadcn_>
    </div>
...
export function useQueryTableCommands(options?: CommandOptions) {
  ...
  const setPage = useSetPage()
  const { t } = useI18n()
  const queryTablePageName = t('sqlCommands.queryTablePage', 'Query a table')
  ...
  useRegisterCommands(
    ...
    [
      {
        id: 'query-table',
        name: t('sqlCommands.queryTable', 'Query a table'),
        icon: () => <Table2 />,
        action: () => setPage(queryTablePageName),
      },
    ],
