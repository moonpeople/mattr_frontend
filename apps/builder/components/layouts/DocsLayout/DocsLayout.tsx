...
function DocsLayout({ title, children }: { title: string; children: ReactElement }) {
  ...
  if (error) {
    return (
      <ProjectLayout product={t('nav.apiDocs', 'API Docs')}>
        <Error error={error} />
      </ProjectLayout>
    )
  }
  ...
  return (
    <ProjectLayout
      title={title || t('nav.apiDocs', 'API Docs')}
      isLoading={isLoading}
      product={t('nav.apiDocs', 'API Docs')}
      productMenu={
        !hideMenu && (
          <ProductMenu
            page={getPage()}
            menu={generateDocsMenu(projectRef, tableNames, functionNames, { authEnabled }, t)}
          />
        )
      }
    >
