...
const IntegrationsLayout = ({ children }: PropsWithChildren) => {
  ...
  return (
    <ProjectLayout
      ...
      product={t('nav.integrations', 'Integrations')}
      isBlocking={false}
      productMenu={
        <>
          <ProductMenu
          ...
          <div className="px-4 py-6 md:px-6">
            <Menu.Group
              title={
                <div className="flex flex-col space-y-2 uppercase font-mono">
                  <span>{t('integrationsMenu.installed', 'Installed')}</span>
                </div>
              }
            />
            ...
            {isError && (
              <AlertError
                ...
                error={error}
                subject={t(
                  'integrationsMenu.errorInstalled',
                  'Failed to retrieve installed integrations'
                )}
              />
            )}
            {isSuccess && (
...
const generateIntegrationsMenu = ({
  ...
  return [
    {
      title: t ? t('integrationsMenu.explore', 'Explore') : 'Explore',
      items: [
        {
          name: t ? t('integrationsMenu.all', 'All') : 'All',
          key: 'integrations',
        ...
        ...(showWrappers
          ? [
              {
                name: t ? t('integrationsMenu.wrappers', 'Wrappers') : 'Wrappers',
          ...
          : []),
        {
          name: t ? t('integrationsMenu.postgresModules', 'Postgres Modules') : 'Postgres Modules',
