...
export const ComputeBadgeWrapper = ({
  ...
  const { data: organization, isPending: isLoadingOrganization } = useOrganizationQuery(
    { slug },
    { enabled: open && typeof slug !== 'undefined' }
  )

  const isEligibleForFreeUpgrade = organization?.plan.id !== 'free' && computeSize === 'nano'

  const isLoading = isLoadingAddons || isLoadingOrganization
