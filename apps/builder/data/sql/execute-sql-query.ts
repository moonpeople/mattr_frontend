...
export async function executeSql<T = any>(
  ...
  const normalizedData = normalizeResultRows(data)
  return { result: (normalizedData ?? data) as T }
}

function normalizeResultRows(data: unknown) {
  ...
  const columnNames = columns.map((col: any, index: number) => {
