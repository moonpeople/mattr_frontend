...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const normalizedCompletionMetadata =
      completionMetadata && typeof completionMetadata === 'object'
        ? completionMetadata
        : {
            textBeforeCursor: req.body.textBeforeCursor ?? '',
            textAfterCursor: req.body.textAfterCursor ?? '',
            prompt: req.body.prompt ?? '',
            selection: req.body.selection ?? '',
          }
    const { textBeforeCursor, textAfterCursor, prompt, selection } = normalizedCompletionMetadata
